...
class ProcessCollection:
    ...
    def gather_missing_unused_data_types(self):
        ...
        for filename in os.listdir(directory):
            ...
            for dictionary in data:
                if(dictionary):
                    pd.gather_dictionary_keys(missing_data_types, dictionary)

            unused_data_types = USED_DATA_TYPES - use_data_types

            self.__data_types_JSON['unused_data_types'][os.path.splitext(filename)[0]] = list(unused_data_types)
            self.__data_types_JSON['missing_data_types'][os.path.splitext(filename)[0]] = list(missing_data_types)
    ...
    def import_marshal_data(self, file_path = "./dataTypes.marshal"):
        ...
        self.__missing_data_types = marshal.load(storage_file)
        storage_file.close()
